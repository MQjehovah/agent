...
class Config:
    ...
    def load_from_env(cls):
        ...
        cls.SEARXNG_URL = s.get("search.searxng_url", "")
        cls.SEARXNG_ENGINES = s.get("search.searxng_engines", "google,bing,duckduckgo")
        cls.SEARXNG_TIMEOUT = int(s.get("search.searxng_timeout", 10))
        cls.TAVILY_API_KEY = s.get("search.tavily_api_key", "")
        cls.SERPER_API_KEY = s.get("search.serper_api_key", "")
        cls.BING_SEARCH_API_KEY = s.get("search.bing_search_api_key", "")
        cls.SEARCH_BACKENDS = s.get("search.backends", "")

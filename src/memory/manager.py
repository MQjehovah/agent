...
class MemoryManager:
    ...
    def _add_user(self, user_id: str, category: str, content: str,
        ...
        if not storage or not user_id:
            logger.warning(f"记忆写入跳过（无 storage 或 user_id）: [{category}]")
            return
        storage.save_memory(
    ...
    def add_preference(self, user_id, content):
        self._add_user(user_id, "preference", content, source="manual")

    def add_key_info(self, user_id, content):

"""
Agent 循环模块 — run_impl / run_impl_reflective / think / execute_tool_calls_parallel
...
async def run_impl(agent, task: str, session_id: str, user_id: str, user_name: str, inherited) -> AgentResult:
    ...
    if agent.client and hasattr(agent.client, "usage_tracker"):
        try:
            agent.client.usage_tracker.flush()
        except Exception as e:
            logger.warning(f"用量 flush 失败: {e}")

    agent.tracer.end_span(status="ok" if ctx.status == "completed" else "error")

    ctx_stats = agent.tracer.get_context_stats()
    if ctx_stats["samples"] > 0:
        logger.info(f"[{agent.name}] [{session.session_id if session else ''}] 上下文统计: "
                    f"峰值={ctx_stats['peak']:,}token, 最终={ctx_stats['final']:,}token, "
                    f"均值={ctx_stats['avg']:,}token, 采样数={ctx_stats['samples']}")

...
class AgentSession:
    ...
    def is_expired(self, ttl_seconds: int = None) -> bool:
        """检查会话是否过期"""
        ttl = ttl_seconds or int(get_settings().get("session.ttl_seconds", 3600))
...
class AgentSessionManager:
    ...
    def __init__(self, ttl_seconds: int = None, max_sessions: int = None):
        ...
        s = get_settings()
        self.ttl_seconds = ttl_seconds or int(s.get("session.ttl_seconds", 3600))
        self.max_sessions = max_sessions or int(s.get("session.max_sessions", 100))

...
export const INSTALL_POLICY_LABELS: Record<string, string> = {
  optional: '可选加入',
  default_on: '默认加入',
  required: '必装'
}
...
export function distName(d?: string): string {
  if (!d) return '云端 + 本地'
  return DISTRIBUTION_LABELS[d] || d
}
export function policyName(p?: string): string {
  return INSTALL_POLICY_LABELS[p || 'optional'] || p || ''
}

export interface Grade {
  label: string
  cls: 'badge' | 'badge-success' | 'badge-primary' | 'badge-warning'
}
/** 质量分级(按评分推导;无评分为「新品」) */

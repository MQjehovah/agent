...
- `config_dir` 指向普通 `PROMPT.md` → `_run_impl()` [ReAct 循环]
- `config_dir` 指向含 `TEAM.md` 的目录 → `_team_run_impl()` [团队编排]
- Agent 可嵌套（父→子代理→孙代理），`contextvars` 保证并发隔离

**文件**：`src/agent.py` (1496 行)
...
SubagentManager.get_or_create_subagent(template)
  ├─ 按 template 名查找活跃 session → 复用
  └─ 创建新 Agent + 新 session → 注册
```
...
TeamOrchestrator.run(task)
  ...
  ├─ DAG 执行引擎
  │   └─ 并行执行就绪节点
  │       └─ _run_stage(role) → _create_team_subagent → Agent.run()
  ├─ 反馈循环 (testing 失败 → implementation 修复 → retest)
  │   └─ 最多 max_loops=3 轮
  ├─ Leader 审核 (requirements/architecture 阶段)
  └─ _build_report() → 执行报告
```
...
```
产品经理 → 软件架构师 → 代码工程师 → 测试工程师
→ 安全审查师 → DevOps工程师 → 文档专员
```

每个成员有自己的 `PROMPT.md` + 共享技能目录 (`skills/`)。角色边界严格（产品经理不写代码，代码工程师不写测试）。

**文件**：`src/subagent_manager.py` (716 行)、`src/team/` (orchestrator/context/dag/feedback/pipeline_builder)
...
**文件**：`src/permissions/`、`src/rbac.py`
...
| 文件 | 职责 |
|------|------|
| `src/agent_session.py` | 4 层压缩管线 |
| `src/tool_result_compressor.py` | 工具结果智能压缩器 |
| `src/prompt.py` | 静态/动态提示词分层 |
| `src/llm.py` | Prompt caching + 本地响应缓存 |
| `src/agent.py` | 压缩管线接入 + 并行工具执行 |
| `src/config.py` | 上下文压缩参数 |
| `src/tracing.py` | 上下文 token 统计 |

...
**文件**：`src/storage/storage.py`（单文件，SQLite 统一存储）
...
```
WebServer (web/server.py + web/auth.py + web/routes.py) — FastAPI
...
`src/tools/compressor.py` 按工具类型注册专用压缩器，替代粗暴的 `result[:3000]` 截断：

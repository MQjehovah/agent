...
| F16 | 多用户 Worker 池(实例隔离) | R-B1/R-B2/R-G3 | ⭕ 已实现、默认关, 建议公司默认开 | worker_pool | P0 |
| F17 | 资源上限/公平调度(worker 容量守护) | R-B3 | ⭕ 容量回收已有,溢出保护待加固 | worker_pool | P1 |
...
| 编号 | 功能 | 需求 | 现状 | 模块 | 优先级 |
|---|---|---|---|---|---|
| F39 | 分层模块化(拆 server/core 巨型文件) | R-I1 | 🕐 目标架构(评审 §5) | web/core | P1 |

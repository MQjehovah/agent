...
| 失败/空结果 | 重新委派，补充指引 |
| **市场专家委派(`market_delegate`)失败/结果不足** | **自动兜底**：用 `market_runtime` 直接调用该专家的依赖能力，或你自行处理 |
...
- **默认不生成报告文件**：任务结果直接在对话中输出，不写任何报告/文档文件。
- **仅当用户明确要求“生成/保存报告文件”时**才生成，且统一写入工作目录下的 `.agent/report/` 子目录（如 `.agent/report/2026年7月经营复盘.md`）。
- 委派任务时，除非用户明确要求保存报告，否则**不要要求子代理落盘报告文件**；子代理只需在对话中返回完整结果。
- 过程/临时文件写 `.agent/tmp/`，不得写入 `.agent/report/`。

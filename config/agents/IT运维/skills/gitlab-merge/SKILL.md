...
```bash
python "E:/ai/agent/config/agents/IT运维/skills/gitlab-merge/gitlab_merge.py" <PROJECT_PATH> <SOURCE_BRANCH> <TARGET_BRANCH>
```

**示例**：将 `cloud/xz-data` 的 `dev` 合入 `test`：
```bash
python "E:/ai/agent/config/agents/IT运维/skills/gitlab-merge/gitlab_merge.py" cloud/xz-data dev test
```

**示例**：将 `cloud/xz-sc50` 的 `test` 合入 `master`：
```bash
python "E:/ai/agent/config/agents/IT运维/skills/gitlab-merge/gitlab_merge.py" cloud/xz-sc50 test master
```
...
2. 读取结果文件获取详情：
```bash
cat "E:/ai/agent/config/agents/IT运维/skills/gitlab-merge/merge_result.txt"
```
3. 将合并结果报告给用户
